...
@CommandLine.Command(name = "quota",
    ...
    description = "Operational tool to repair OM quota.")
...
public class QuotaRepair implements Callable<Void>, SubcommandWithParent {
  ...
  private OzoneConfiguration getOzoneConf() {
    // Navigate up the command hierarchy to get to OzoneRepair
    CommandLine cmd = spec.commandLine();
    while (cmd.getParent() != null) {
      cmd = cmd.getParent();
    }
    Object root = cmd.getCommand();
    if (root instanceof OzoneRepair) {
      return ((OzoneRepair) root).getOzoneConf();
    }
